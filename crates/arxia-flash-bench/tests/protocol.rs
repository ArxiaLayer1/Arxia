...
use arxia_flash_bench::{
    audit, boot, report_audit, run_one, tag_of, AuditReport, BatchOutcome, BenchBatch,
    BootDecision, Verdict, HIGH_WATER_KEY,
};
...
/// Every value the bench writes carries its sequence, and every key
/// of a sequence is distinct from every key of every other: the two
/// properties the audit's per-sequence walk depends on.
#[test]
fn every_value_is_tagged_and_every_sequence_has_its_own_keys() {
    ...
    for v in &b.values {
        assert_eq!(tag_of(v), Some(8));
    }
    for ka in &a.keys {
        for kb in &b.keys {
            assert_ne!(ka, kb, "keys of different sequences must differ");
        }
    }
...
// ------------------------------------------ the boot's outcome classes

/// The batch loop reports the trait's classes on the serial line: a
/// lying commit on an otherwise healthy flash finishes inline and says
/// "ok", and the audit finds the batch complete.
#[test]
fn a_lying_commit_finishes_inline_and_audits_complete() {
    // Find the smallest cut budget at which sequence 2 lands: that
    // budget is one past the marker write.
    let total = {
        let flash = FaultyFlash::healthy();
        let mut out = sink();
        let (mut store, _) = boot(flash, RANGE, &mut out).expect("mount");
...
fn a_committed_but_unfinished_batch_is_reported_committed_not_refused() {
    // Same committing-budget search as the lying-commit fixture.
    let total = {
        let flash = FaultyFlash::healthy();
        let mut out = sink();
        let (mut store, _) = boot(flash, RANGE, &mut out).expect("mount");
