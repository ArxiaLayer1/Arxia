...
use arxia_transport::traits::{SignedTransportMessage, TransportError, TRANSPORT_FRESHNESS_WINDOW};
...
fn fresh_message_is_accepted() {
    ...
    assert!(signed(now)
        .verify_at(now, TRANSPORT_FRESHNESS_WINDOW)
        .is_ok());
}
...
fn message_at_the_exact_window_edge_is_accepted() {
    let now = 1_700_000_000_000u64;
    let m = signed(now - TRANSPORT_FRESHNESS_WINDOW.as_millis() as u64);
    assert!(m.verify_at(now, TRANSPORT_FRESHNESS_WINDOW).is_ok());
}
...
fn replayed_message_is_rejected() {
    ...
    assert!(
        matches!(
            m.verify_at(now, TRANSPORT_FRESHNESS_WINDOW),
...
fn huge_timestamp_does_not_overflow() {
    ...
    assert!(matches!(
        m.verify_at(1_000, TRANSPORT_FRESHNESS_WINDOW),
        Err(TransportError::TimestampStale { .. })
    ));
}
